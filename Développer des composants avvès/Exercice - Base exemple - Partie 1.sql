...
SELECT nom, nodep FROM employe WHERE nopdep = 40
...
SELECT salire, nom FROM employe WHERE service = 41 ORDER BY salaire ASC

--19.Afficher le salaire et le nom des employés du service 41 classés par salaire décroissant.
SELECT salaire, nom FROM employe WHERE service = 41 ORDER BY salaire DESC
...
SELECT nom, UPPER(nom), LOWER(nom) FROM employe WHERE nom LIKE "Vrante"
